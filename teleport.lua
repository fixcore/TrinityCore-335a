...
teleport.WrongPassText = "Error de clave!"

teleport.ports = {
    {name = "Location 1", mapid = 1, x = 2, y = 3, z = 4, o = 5},
    {name = "Location 2", mapid = 1, x = 2, y = 3, z = 4, o = 5, pass = "km"},  --clave
...
function teleport.BuildMenu(Unit, Player, from)
    ...
    if from > 0 then
        ...
        Player:GossipMenuAddItem(7, "Atras..", 0, GoBack)
    end
...
function teleport.OnTalk(Event, Player, Unit, _, ID, Password)
    if Event == 1 or ID == 0 then
        ...
        if not M then error("Esto no debería ocurrir") end
